...
// RenderTournamentHTML renders a self-contained merit-ranking page with the
// trust stats (flip rate, cycle rate, grounded rate) that say how much to
// believe the order. judgeName labels the judge; note carries caveats.
func RenderTournamentHTML(t TournamentResult, judgeName, note string) string {
	...
	b.WriteString("<h1>Merit tournament — best read wins</h1>")
	fmt.Fprintf(&b, `<p class="sub">Pairwise "which is the better read", judged by %s in both slot orders (a decisive win must survive the swap). Ranked by order-swapped merge sort; ties keep the composite seed.</p>`, html.EscapeString(judgeName))
	...
	if len(t.Edges) > 0 {
		b.WriteString(`<h2 style="font-size:1.1rem;margin-top:1.6rem">Comparisons</h2><table><thead><tr><th>A</th><th>B</th><th>Winner</th><th>Judge reason</th></tr></thead><tbody>`)
		for _, e := range t.Edges {
			...
			fmt.Fprintf(&b, `<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>`,
				html.EscapeString(e.A), html.EscapeString(e.B), win, html.EscapeString(e.Reason))
		}
		b.WriteString(`</tbody></table>`)
	}
