...
// RenderHTML renders a self-contained routing table for Go tracing accuracy.
func RenderHTML(scores []ModelScore) string {
	...
	b.WriteString(`<title>distill — Go trace routing</title><style>`)
	...
	b.WriteString("<h1>Go trace routing — exact stdout wins</h1>")
	b.WriteString(`<p class="sub">Deterministic program-tracing accuracy against trusted go-run gold (no LLM judge; model output is not executed).</p>`)
	...
	if len(scores) > 0 {
		...
		fmt.Fprintf(&b, `<div class="stat"><b>%d/%d</b><span>correct</span></div>`, top.Correct, top.N)
		b.WriteString(`</div>`)
	}

	b.WriteString(`<table><thead><tr><th>Rank</th><th>Model</th><th class="num">Accuracy</th><th class="num">Correct</th><th class="num">Unparseable</th><th class="num">Skipped</th><th class="num">ms/task</th>`)
	if showCost {
	...
	for i, s := range scores {
		...
		fmt.Fprintf(&b, `<tr><td class="num">%d</td><td>%s</td><td class="num">%.1f%%</td><td class="num">%d/%d</td><td class="num">%d</td><td class="num">%d</td><td class="num">%.0f</td>`,
			i+1, html.EscapeString(s.Model), 100*s.Accuracy, s.Correct, s.N, s.Unparseable, s.Skipped, perTask)
		if showCost {
			if s.CostPerMTok > 0 {
	...
	b.WriteString(`<h2>Task details</h2>`)
	b.WriteString(`<table><thead><tr><th>Model</th><th>Task</th><th>Category</th><th>Status</th><th class="num">ms</th><th>Gold</th><th>Prediction / error</th></tr></thead><tbody>`)
	for _, s := range scores {
		for _, p := range s.Predictions {
			...
			fmt.Fprintf(&b, `<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td class="num">%d</td><td>%s</td><td>%s</td></tr>`,
				html.EscapeString(s.Model),
				html.EscapeString(p.ID),
				html.EscapeString(p.Category),
				html.EscapeString(p.Status()),
				p.ElapsedMS,
				html.EscapeString(p.Gold),
